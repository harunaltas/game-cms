<x-layout bodyClass="g-sidenav-show bg-gray-200">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <!-- Navbar -->
        <x-navbars.navs.auth titlePage="Kullanıcı Düzenle"></x-navbars.navs.auth>
        <!-- End Navbar -->
                    ...
                    <div class="card my-4">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                            <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
                                <h6 class="text-white mx-3"><strong>Kullanıcı Düzenle</strong> </h6>
                            </div>
                        </div>
                        ...
                        <form method="post" action="{{ route('users.update', $user->id) }}">
                            @csrf
                            @method('PUT') <!-- Bu satırı ekleyin -->
                            <div class="row px-5">
                                <!-- Sol Taraf: Kullanıcı Adı, E-Mail, Şifre (Şifre alanı opsiyonel) -->
                                <div class="col-md-6">
                                    <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    <label>Güvenlik Sorusu Seçin</label>
                                        @foreach($questions as $key => $question)
                                            <div class="form-check">
                                                <input class="form-check-input" type="radio" name="security_question" id="question{{ $key }}" value="{{ $key }}" {{ $user->security_question == $key ? 'checked' : '' }}>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="security_answer" class="form-control border border-2 p-2" value="{{ $user->security_answer }}">
                                    </div>
                                </div>
                            </div>
                            <div class="row">
